function moduleProject1() {
  ...
  // create the quotes elements
  const quote = document.createElement("div");
  const quoteText = randomQuote.quote;
  quote.textContent = quoteText;
  const widget1 = document.querySelector(".quoteoftheday");
  widget1.appendChild(quote);
  // create the author and date elements
  ...
  const randomWords = `We need to ${randomVerb1} our ${randomNoun1} ${randomAdverb1} in order to ${randomVerb2} our ${randomNoun2} ${randomAdverb2}.`;
  const paragraph = document.createElement("p");
  paragraph.textContent = randomWords;
  document.querySelector(".corporatespeak").appendChild(paragraph);

  // 👉 TASK 4 - Build a "Countdown" widget
  //  ✨ add your code here
  const timerWidget = document.querySelector(".countdown");
  let count = 5;
  const timer = document.createElement("p");
  timer.textContent = `T-minus ${count}...`;
  timerWidget.appendChild(timer);
  ...
  //  ✨ add your code here
   ...
   let sentence = `${person.fname} ${person.lname} was born in ${year} `;
